...
Compare lad tables-- Note: with introduction of NA's via DWR/CBR method, many rows are mismatched-- not an accurate assessment. Instead compare final run designation. Keeping just for reference

```{r compare_LAD_designation_tables, eval = FALSE, include=FALSE}
...
#will update but keeping all matches returned to confirm function is working properly which causes all NAs to return
df_catch_designation_cbr <- fct_LAD_designation(lad_data = lad_cbr_long, catch_data = df_catch) #returns all NA matches-- extract run via:
  #Remove NA's and extact run
  df_catch_designation_cbr <- df_catch_designation_cbr %>%
    mutate(run_designation = str_extract(run_designation, "\\b(?!NA\\b)[^,]+")) 
...
Differing thresholds between FWS and DWR/CBR (with updated NA's in DWR/CBR method):
...
df_different_runs_cbr.fws 
```

Beginning plots to see differences 
