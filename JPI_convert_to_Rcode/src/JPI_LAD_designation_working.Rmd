...
Compare lad tables
Note: min/max1 seems to be switched for minmax2 for some of cbr matrix. Code at the bottom of this chunks rearranged to match lad_fws organization. Leaves data with 98 rows of mm/dd/run that differ between the two tables. 
...
# Find the rows that only appear in one data frame
...
#seems that the maxmin are just mirrored in a lot of diff_rows---resort to match up
#rename columns to resort
...
# Remove the rows from lad_cbr_long that are in diff_rows_recode
