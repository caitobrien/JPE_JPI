...
- Figure 5 (fig 6 on Track a cohort_WR_plots.docx) Cumulative loss by type - streamline code and update data sourcing
...
```{r append_hydro_year, include=FALSE}
...
color_gradient <- colorRampPalette(c("cadetblue3", "brown"))(length(unique(wytype$`Yr-type`)))

# Define all possible levels of hydro_type
all_levels <- c("Wet", "AboveNormal", "BelowNormal", "Dry", "Critical")
...
p_stars_total_survival <- ggplot(df_stars, aes(x = wDay, group = WY, color =hydro_type)) +
