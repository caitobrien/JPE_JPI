...
**Background**
Objective: Using shared resources via [BDO github](https://github.com/BDO-Science/track-a-cohort) from BOR, include more interactive capabilities to certain graphs in form of Rmarkdown and/or shinyapp. See 
[Track a cohort_WR_plots.docx](https://github.com/BDO-Science/track-a-cohort/blob/main/Track%20a%20Cohort_WR_plots.docx) for figures requested. 

Current figures working on:

- Figure 2. STARs survival - shiny and update data sourcing (NB/SI)

- Figure 3a & 3b JPE loss (a) percent and b) count) (3b is Fig 4 on Track a cohort_WR_plots.docx)- minor improvements & update data sourcing

- Figure 4 (Fig 3b on Track a cohort_WR_plots.docx) Winter-run Current & Historical Cumulative Salvage - highlight years compared to current year and update data sourcing
...
library(CDECRetrieve) #download cdec hydrological classification table
```

```{r include=FALSE}
CDECRetrieve::water_year_indices
```


**Current figures**

## Figure 2. STARS Model Estimates 
...
#get hydrological classification table
hydro_type<-CDECRetrieve::water_year_indices %>% 
  filter(location == "Sacramento Valley") %>% 
  select(wy, yr_type)

#combine hydro year infor with stars data
df_stars<-df_stars_raw %>% 
  left_join(hydro_type, by = c("WY" = "wy"))
  

#If wanting late_fall 
...
```{r p_base_stars_total_survival, eval = FALSE, include=FALSE}
p_stars_total_survival <- ggplot(df_stars, aes(x = wDay, group = WY, color = yr_type)) +
...
fct_stars_survival_plot(data = df_stars, x_var = "doy", year = "CY", metric = "idRoute")
...
```{r p_fig4a_all_years, echo=FALSE, message=FALSE, warning=FALSE, fig.cap= "Black line represents the current WY (2024) and the grey lines represent previous years (WY1994 to WY2023)"}
#plot
...
p1<-cumulativeLAD %>% 
 ggplot() +
  ...
  labs(x = 'Date', 
       y = 'Cumulative Loss', 
       title = 'Winter-run Current and Historical Cumulative Salvage') +
  scale_x_continuous(breaks = c(93,152,213,274), labels = c('Jan', 'Mar', 'May', 'Jul')) +
...
```{r p_fig4b_facet, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=8, fig.cap= "Black line represents all years above the current WY (2024) max cumulative loss and the grey lines represent previous years below WY2024 cumulative loss. Data includes: WY1994 to WY2024"}
...
p2 <- cumulativeLAD %>%
  ...
  geom_text(data = year2001_data, aes(x = 197, y = 9500, label = "*2001"), size = 3,fontface = "plain") +
  labs(x = 'Date', 
       y = 'Cumulative Loss', 
       title = 'Winter-run Current and Historical Cumulative Salvage') +
  scale_x_continuous(breaks = c(93,152,213,274), labels = c('Jan', 'Mar', 'May', 'Jul')) +
