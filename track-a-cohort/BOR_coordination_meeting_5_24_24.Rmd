...
df_loss_sum <- df_loss_sum %>% 
  inner_join(select(df_jpe,WY,brood_year, "JPE" = value ), by = "WY" )
...
```{r p_cbr, include=FALSE, fig.show='hold', out.width='50%'}
 ggplot(data=df_loss_sum, aes(x=as.factor(brood_year), y= pctloss*100, fill=losstype)) +
  geom_bar(stat="identity", position=position_dodge())+
  labs(x = "Brood Year", 
       y = "Percent of JPE Loss", #change title
       fill = NULL) +
...
```{r fig3b_shared_code, include=FALSE}
library(busdater)
...
p2 <- cumulativeLAD %>%
  ...
  labs(x = NULL, 
       y = 'Cumulative Loss', 
       # title = 'Winter-run Chinook salmon -  Current and Historical Cumulative Salvage',
       subtitle = "Pre-2009 BiOp: 1994 to 2008") +
  scale_x_continuous(breaks = c(93,152,213,274), labels = c('Jan', 'Mar', 'May', 'Jul')) +
...
p3<-  cumulativeLAD %>%
 ...
 add_row(WY = NA, Status = NA, hydro_type = "Above Normal", cumloss = NA, wDay = NA) %>%
  ...
  labs(x = 'Date', 
       y = 'Cumulative Loss', 
       title = NULL,
       subtitle = "2009 & 2019 BiOp: 2009 to 2022",
...
- Keep CDEC naming or 5 hydro classifications or set all to either "Dry" or "Wet"? 
