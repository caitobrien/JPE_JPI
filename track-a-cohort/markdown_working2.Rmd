...
df_jpe_loss_sum <-  jpe %>% 
  # mutate(WY = brood_year + 1) %>% 
  select(WY, brood_year, method, value ) %>% 
  inner_join(df_loss_sum, by = "WY") 
  

df_pct_jpe_loss_sum<-df_jpe_loss_sum %>% 
  group_by(WY, losstype) %>% 
  mutate(pctloss = (nloss/value))

  # ggplot(data=df_pct_jpe_loss_sum, aes(x=as.factor(brood_year), y= pctloss, fill=losstype)) +
...
pct_cumloss<-cumulativeLAD %>% 
  group_by(WY, wDay, hydro_type_grp) %>% 
  summarise(cumloss = max(cumloss)) %>% 
  inner_join(select(jpe,WY, value), by = "WY") %>% 
  mutate(pct_cumloss = cumloss/value)
```
...
```{r p_fig3a_all_years, echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.width=10}
#plot
...
cumulativeLAD %>% 
 ggplot() +
  geom_line( data = subset(cumulativeLAD, WY != 2024), aes(x=wDay, y = cumloss, group = WY, color = hydro_type_grp ), lwd = .25)+
  ...
  scale_color_manual(values = c("brown", "cadetblue3", "grey"), 
                     labels = c("Below Normal, Dry, & Critical", "Wet, Above Normal", "WY2023"))+
